...
**Scaffold only:** every proof is `sorry`; carriers are placeholders.
-/

namespace PrimesXNY2.RingClassField

open PrimesXNY2.Order

/-- The **Hilbert class field** `H` of an imaginary quadratic field `K`: its
maximal unramified abelian extension. Carrier placeholder. (Cox, §5.) -/
def hilbertClassField (O : QuadOrder) : Type := sorry

/-- The **ring class field** of the order `𝒪` of conductor `f`: the abelian
extension `L/K` whose Galois group is `C(𝒪)` via the Artin map. Carrier
placeholder. (Cox, §9.) -/
def ringClassField (O : QuadOrder) : Type := sorry

/-- Placeholder for `Gal(L/K)`, the Galois group of the ring class field over `K`.
(Cox, §8.) -/
...
/-- A prime `p` **splits completely** in the ring class field of `𝒪`. Placeholder
